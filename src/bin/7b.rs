...
// const INPUT: &'static str = "aba[bab]xyz
// xyx[xyx]xyx
// aaa[kek]eke
// zazbz[bzb]cdb";

const INPUT: &'static str = "dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
fn main() {
  let between = ::regex::Regex::new(r"\[.*?\]").unwrap();

  let count = INPUT.lines()
    .filter(|l| {
      let abasv = between.split(l).map(|s| s.chars().collect::<Vec<_>>().windows(3).filter(|w| w[0] != w[1] && w[0] == w[2]).map(|w| (w[0], w[1])).collect::<Vec<_>>());

      abasv.flat_map(|v| v.into_iter()).any(|(a, b)| between.captures_iter(l).any(|c| c.at(0).unwrap().chars().collect::<Vec<_>>().windows(3).any(|w| w[0] == b && w[1] == a && w[2] == b)))
    })
    .count();
