...
// const INPUT: &'static str = "abba[mnop]qrst
// abcd[bddb]xyyx
// aaaa[qwer]tyui
...
fn abba(s: &str) -> bool {
  ...
  chars.windows(4).any(|w| w[0] != w[1] && w[0] == w[3] && w[1] == w[2] && w[0] != '!' && w[1] != '!')
}
...
fn main() {
  ...
  let count = INPUT.lines()
    .filter(|l| between.split(l).any(|s| abba(&s)) && !between.captures_iter(l).any(|c| abba(&c.at(0).unwrap())))
    .count();
