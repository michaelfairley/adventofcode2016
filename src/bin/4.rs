...
const INPUT: &'static str = "vxupkizork-sgmtkzoi-pkrrehkgt-zxgototm-644[kotgr]
...
// const INPUT: &'static str = "aaaaa-bbb-z-y-x-123[abxyz]
...
fn main() {
  ...
  for line in INPUT.lines() {
    ...
    let sector = split.next().unwrap().parse::<i32>().unwrap();
    let checksum = &split.next().unwrap()[0..5];

    let mut counts = HashMap::new();

    for ch in name.chars().filter(|&c| c != '-') {
      let count = counts.entry(ch).or_insert(0);
      *count -= 1; // Smaller numbers come first
    }

    let mut ordered = BTreeSet::new();

    for (ch, count) in counts {
      ordered.insert((count, ch));
    }

    let firstfive = ordered.iter().map(|&(_,ch)| ch).take(5).collect::<String>();
